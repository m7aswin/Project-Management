...
const HomePage = () => {
  ...
  useEffect(() => {
    const getAllMaterials = async () => {
      try {
        ...
        setLoading(true);
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/materials/getAll`);
        setLoading(false);
  ...
  const handleDelete = async (record) => {
    try {
      setLoading(true);
      await axios.post(`${process.env.REACT_APP_API_URL}/materials/delete-material`, {
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      setLoading(true);
      if (editable) {
        await axios.post(`${process.env.REACT_APP_API_URL}/materials/edit-material`, { ...values });
        setEditable(values);
        let tempData = allMaterials;
        ...
        await axios.post(`${process.env.REACT_APP_API_URL}/materials/add-material`, {
          ...values,
        });
